...
const CartPage = () => {
  ...
  if (status === "loading") return <p className="text-white p-10">Cargando sesión...</p>;
  if (status === "unauthenticated") return <p className="text-white p-10">Debes iniciar sesión.</p>;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-slate-950 to-slate-900 text-white px-6 py-14">
      ...
      {cart.length === 0 ? (
        <p className="text-center text-gray-400">No tienes productos en el carrito.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {cart.map((item) => (
            ...
            >
              ...
              <h3 className="text-xl font-bold text-white text-center">{item.productId.name}</h3>
              <p className="text-gray-400 text-sm italic">{item.productId.category}</p>
              <div className="flex justify-between w-full items-center">
                <span className="font-semibold text-violet-200">${item.productId.price.toFixed(2)}</span>
      ...
      {cart.length > 0 && (
        <div className="mt-10 text-right">
          <p className="text-xl font-bold text-gray-200">
            Total: <span className="text-cyan-400">${totalPrice.toFixed(2)}</span>
          </p>
        </div>
      )}
    </div>
